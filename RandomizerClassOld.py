...
class Randomizer:
    ...
    def rename_files(self):  # we need to give a temp name so that we dont create files with the same name
        ...
        for f in listdir(self.directory):  # first we set the file name to a sample number because we can't have two file names
            if isfile(join(self.directory, f)) and not f.split("y")[0] == "zqx" and not f.endswith(".mcmeta"):  # check if it is a file and does not have name starting with z before the first y (Not a temp file name)
                ...
                os.rename(self.directory + "\\" + f, self.directory + f"\\zqxy{iterations}.txt")  # the brackets allow us to use ints in a string (using a + or comma breaks the funtion)
                iterations += 1
        print(self.files)
        iterations = 0
        for f in listdir(self.directory):  # now we can rename all files with the random names without errors of two files with the same name
            ...
            if isfile(join(self.directory, f)) and f.split("y")[0] == "zqx" and iterations < len(self.files):
